...
all_trt <- all_trt_ln[complete.cases(all_trt_ln),]
...
for(i in 2:length(baseline_corr1)){
  b1= ggplot(baseline_corr1, aes(x=M_VALUE, y=baseline_corr1[,i])) + geom_point()+
    ...
    ggtitle(paste("Baseline Comparison for ln(M-value) v ln(",colnames(baseline_corr1)[i],")")) + ylab(colnames(baseline_corr1)[i])
  b[[i]] <- b1
}
pdf("Baseline_ln_plots.pdf")
for(i in 2:length(baseline_corr1)){
  print(b[[i]])
}
dev.off()
...
for(i in 2:length(endpoint_corr1)){
  e1= ggplot(endpoint_corr1, aes(x=M_VALUE, y=endpoint_corr1[,i])) + geom_point()+
    ...
    ggtitle(paste("Baseline Comparison for ln(M-value) v ln(",colnames(endpoint_corr1)[i],")")) + ylab(colnames(endpoint_corr1)[i])
  e[[i]] <- e1
}
pdf("Endpoint_ln_plots.pdf")
for(i in 2:length(endpoint_corr1)){
  print(e[[i]])
}
dev.off()
...
for(i in 2:length(change_corr1)){
  c1= ggplot(change_corr1, aes(x=M_VALUE, y=change_corr1[,i])) + geom_point()+
    ...
    ggtitle(paste("Baseline Comparison for ln(M-value) v ln(",colnames(change_corr1)[i],")")) + ylab(colnames(change_corr1)[i])
  c[[i]] <- c1
}
pdf("Change_ln_plots.pdf")
for(i in 2:length(change_corr1)){
  print(c[[i]])
}
dev.off()
